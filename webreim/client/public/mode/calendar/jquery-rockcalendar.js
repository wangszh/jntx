(function ($) {
	...
	function calendarclass(element, options){
		...
		this.init = function(){
			...
			s+='<table width="100%" border="0" style="border-collapse:collapse" height="100%" cellspacing="0" bgcolor="#ffffff" cellpadding="0">';
			s+='<tr>';
			for(var i=0;i<7;i++){
			...
			for(var j=1;j<=6;j++){
				...
				for(var i=0;i<7;i++){
					x++;
					s+='<td align="'+can.align+'" height="'+(100/6)+'%" valign="'+can.valign+'" width="'+w+'%" class="tdtext" id="calcontabc'+x+'_'+id+'" temp="'+x+'"></td>';
				}
				s+='</tr>';
			} 
			s+='</table></div>';
