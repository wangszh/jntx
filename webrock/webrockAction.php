...
class Action extends mainAction
{
	...
	public function exceldown($arr,$table)
	{
		...
		//处理fininfom导出后的标记 start		
		if($table == 'fininfom'){
			$sql="";
			foreach($rows as $k=>$rs){
				$sql.=",".$rs['id'];				
			}
			$sql="isemp=0 and (`id` in (".substr($sql,1)."))";
			m($table)->update("`isemp`=1", $sql);
			//echo $sql;
			//exit;
		}		
		//处理fininfom导出后的标记 end		
...
function f($num)
{
	...
	$cls	= null;
	if(file_exists($path)){
		c('flow', false);//flowChajian.php //加载工作流插件   不返回加载的插件对象
		include_once($path);
		$cname = $num.'flowClass';
		$cls = new $cname($num);//加载flowChajian的子类   并把工作流号传递给flowChajian的初始化函数
	}else{
		$cls = c('flow', true, $num);//flowChajian.php //加载工作流插件   返回加载的类
	}
	return $cls;
}