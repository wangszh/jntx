...
rock['up'+rand+''] = function(a, typen, typenid, sid){
	js.msg('wait','保存文件中...');
	bool = true;
	js.ajax(js.getajaxurl('savedoc',mode,dir),{fileid:sid, typename:decodeURI(typen),atype:atype,typeid:typenid}, function(da){
		js.msg('success', '保存成功');
		grid.storereload();
		bool = false;
	},'post');
...
function uploadfield(){
	...
	uploadwindows.upload('rock.up'+rand+'', tree.changedata.name,{params1:ti,params2:tree.changedata.id});
}
