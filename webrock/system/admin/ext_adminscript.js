...
var panel = {
	...
	columns:[{
		...
		text:'收款账号',dataIndex:'cardid',width:80,search:true
	},{
		text:'身份证',dataIndex:'idnum',width:80,search:true
	},{
		...
		text:'直属上级',dataIndex:'superman',width:80,search:true,autowidth:true,hidden:true
	},{
		...
		text:'所属单位',dataIndex:'companyname',autowidth:true,hidden:true
	},{
	...
	formparams:{
		...
		submitfields:'user,pass,name,idnum,cardid,tel,email,status,groupname,gender,mobile,type,zwid,ranking,superman,superid,deptid,deptname,sort,companyid,weixinid',
		params:{int_filestype:'status,type,zwid,deptid,sort,companyid',add_otherfields:'adddt={now},workdate={date},state=2',md5_filestype:'pass',otherfields:'optdt={now},optname={admin},optid={adminid}'},autoScroll:false,editrecord:true,
		...
		items:[{
			...
			xtype: 'fieldcontainer',defaultType: 'textfield',layout:'hbox',items:[{
				fieldLabel:''+bitian+'身份证',name:'idnumPost',allowBlank: false,width:'49%'
			},{
				fieldLabel:''+bitian+'收款账号',name:'cardidPost',allowBlank: false,width:'50%'
			}]
		},{
			xtype: 'fieldcontainer',defaultType: 'textfield',layout:'hbox',items:[{
			...
			xtype: 'fieldcontainer',defaultType: 'textfield',layout:'hbox',items:[{
				...
				fieldLabel:''+bitian+'所属部门',nameidfields:'deptid_'+rand+'',name:'deptnamePost',xtype:'changedeptuser',changetozw:false,changetitle:'选择部门',allowBlank: false,width:'50%',listeners:{change:function(o){
					form.field("rankingPost").setDisabled(false);
				}}
			},{				
				name:'zwidPost',id:'zwid_'+rand+'',hidden:true
			},{				
				fieldLabel:''+bitian+'职位',nameidfields:'zwid_'+rand+'',name:'rankingPost',xtype:'changedeptuser',changetitle:'选择职位',changetozw:true,disabled:true,changetopdid:0,width:'50%',changecheck:function(o){
					var did=form.field("deptidPost").getValue();
					//did=4;//test
					if(did){
						//alert(form.field("deptidPost").getValue());
						form.field("rankingPost").setchangetopdid(did);
						//alert(form.field("rankingPost").getchangetopdid());
						return true;
					}else{
						return false;
					}
			...
			xtype: 'fieldcontainer',defaultType: 'textfield',layout:'hbox',items:[{
				fieldLabel:'直属上级',nameidfields:'superid_'+rand+'',name:'supermanPost',xtype:'changedeptuser',changetozw:false,changetitle:'选择对应的直属上级',changetype:'usercheck',width:'49%'
			},{
			...
			xtype: 'fieldcontainer',defaultType: 'textfield',layout:'hbox',items:[{
				fieldLabel:''+bitian+'所在组',name:'groupnamePost',xtype:'checkcombo',editable:false,store:[['1','管理员']],width:'49%'
			},{
