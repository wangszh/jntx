...
function clickedit(){
	...
	if(changedata.checktype=='group')form.field('groupnamePost').setValue(changedata.groupname);
}
...
var panel= [{
	...
	submitfields:'name,num,checktype,atype,atypeid,atypename,pid,nid,checktype,checktypeid,groupname,checktypename,explain,sort,inputid,zscheck,type',
	params:{otherfields:'optdt={now},setid='+setid+'',int_filestype:'pid,nid,sort,inputid,zscheck,type'},autoScroll:false,
	...
	items:[{
		...
		fieldLabel:''+bitian+'指定值',value:'所有人员',nameidfields:'atypeid_'+rand+'',name:'atypenamePost',xtype:'changedeptuser',changetozw:false,changetitle:'选择部门/人员',hidden:true,changetype:'deptcheckuser',changecheck:function(o){
		...
		fieldLabel:'<font color=red>*</font>审核人类型',name:'checktypePost',editable:false,xtype:'optioncombo',optionmnum:'flowchecktype',autoloadlist:true,valuefields:'value',allowBlank: false,change:function(o){
			...
			form.field('groupnamePost').setVisible((val=='group')?true:false);
		...
		fieldLabel:''+bitian+'所在组',name:'groupnamePost',xtype:'rockcombo',url:js.getajaxurl('getgroup','group','system'),allowBlank: false,valuefields:'name',autoloadlist:true,hidden:true
	},{
		...
		fieldLabel:'进程类型',name:'typePost',value:'0',editable:false,xtype:'combo',store:[['0','默认'],['1','可转移给其他人']]
	},{
	...
	submitcheck:function(o){
		...
		if(ctype=='rank'){
			...
			var rank = o.field('groupnamePost').getValue();
			if(isempt(rank)){
				return '请选择组';
			}else{
				ctynae.setValue(rank);
...
var c = {
	...
	show:function(){
		...
		var pap=Raphael('raphael_'+rand+'',w, 3000);//设置高度
		json = {};
		sjson= {};
