...
class flow_courseClassModel extends Model
{
	...
	public function getstatuss($tabs, $rs, $sp=',')
	{
		...
		if(isset($rs['isturn'])&&$rs['isturn']!=1)return '<font color=#ff6600>待提交</font>';
		$ztname = '';
		$status = 1;
		if(isset($rs['status']))$status = $rs['status'];
		if($status==5){
			$ztname = '<font color=#888888>已删除</font>';
			return $ztname;
		}
		$lostlog = m('flow_log')->getone("`table`='$tabs' and `mid`='".$rs['id']."' and courseid>0 order by `id` desc");
		$cols 	 = array('','green','red','green','','','','','','');
		if(is_array($lostlog)){			
		...
		if(isset($rs['nowcheckname']))if(!$this->isempt($rs['nowcheckname']) && $status!=1){
			if($ztname!='')$ztname.=$sp;
			if($status==0){
		...
		if($status==1 && $ztname=='')$ztname='<font color=green>审核完成</font>';
		return $ztname;
	}
}