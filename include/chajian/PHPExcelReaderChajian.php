...
class PHPExcelReaderChajian extends Chajian{
	...
	public function reader($filePath=null, $index=2)
	{
		...
		$rows		= array();
		$sheet 		= $PHPExcel->getSheet(0);//得到工作表
		$allColumn 	= $sheet->getHighestColumn();//得到总列标记 如：Ｆ
		$allRow 	= $sheet->getHighestRow();//得到总行数
		$allCell	= $this->AT[$allColumn];//得到列的数字标记
				
		//echo 'allRow->'.$allRow.'<br>';
		//echo 'allColumn->'.$allColumn.'<br>';
		//echo 'allCell->'.$allCell.'<br>';		
		//return;
