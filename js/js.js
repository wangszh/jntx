...
*/
js.IsDate=function(sm,mystring) {
	var reg = /^(\d{4})-(\d{2})-(\d{2})$/;
	var str = mystring;
...
js.cropimg=function(aid,at,w,h){
	...
	var url='mode/cropimg/cropimg.php?imgsize='+w+'x'+h+'&imgurl='+encodeURI(img)+'&thumimg='+encodeURI(sou)+'&title='+at+'&aid='+aid+'&callback=js.cropimgcall';
	js.open(url,750,430);
	return false;
}
...
js.downshow=function(id){
	js.open('mode/upload/uploadshow.php?id='+id+'',600,350);
	return false;
}
...
js.downupshow=function(a, showid, sid, sdesa){
	var s = '',i=0,s1='';
	var o = $('#view_'+showid+'');
	for(i=0; i<a.length; i++){
		...
		if(!sdesa)s+=' <a class="a" temp="dela" onclick="return js.downupdel('+a[i].id+',\''+showid+'\', this)" href="javascript:">×</a>';
		s+='</div>';
		o.append(s);
	}
...
js.getajaxurl=function(a,m,d,can){
	...
	if(jga=='$'){}else if(jga!='@'){url+='&ajaxbool=true&rnd='+Math.random()+''}	
...
js.tanbody=function(act,title,w,h,can1){
	...
	for(var i=0; i<can.btn.length; i++){
		var a	= can.btn[i];
		s+='<a class="webbtn btn-success" id="'+act+'_btn'+i+'" onclick="return false" href="javascript:">';
		if(!isempt(a.icons))s+='<i class="icon-'+a.icons+'"></i>&nbsp; ';
		s+=''+a.text+'</a>&nbsp; ';
	}
	s+='		<a class="webbtn" id="'+act+'_cancel" onclick="return js.tanclose(\''+act+'\',\''+can.guanact+'\')" href="javascript:"><i class="icon-remove"></i>&nbsp;取消</a>';
	s+='	</div>';
	s+='</div>';
